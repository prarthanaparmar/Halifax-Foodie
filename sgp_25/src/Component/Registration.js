...
function Registration(props) {
    ...
    const withdrawSubmit = (e) => {
        ...
        if (!validName && !validNumber && !validEmail && !validPassword && !validLocation) {
            document.getElementById("password").style.border = "1px solid red"
            document.getElementById("email").style.border = "1px solid red"
                        ...
                        if (validPassword) {
                            if (validLocation) {
                                ...
                                try {
                                    ...
                                    console.log("I am at register page")
                                    
                                    //history.push('/verify');
                                    history.push({
                                        pathname: '/verify',
                                        myCustomProps: { data : user.email }
                                    })
                                } catch (error) {
    ...
    return (
        ...
        <section class='bg-whole-page'>
            ...
            <h1>Register</h1>
            {getEmail.map((getEmail)=> (
                <div key = {getEmail.id}>
                    <h2>{getEmail.email}</h2>
                    <h2>{getEmail.password}</h2>
                </div>
            ))}
